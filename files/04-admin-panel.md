...
Fetch всички данни server-side, показвай с карти:
```
[Icon] Статии        [Icon] Позиции
42 published          8 active
12 drafts            2 inactive
```
...
- Timeline на status промените (ако трябва — за MVP не е задължително)
...
**За бъдещо развитие (placeholder):**
- User management (списък потребители, промяна на роли)
...
- Title, content, footer с actions
- Close on escape и overlay click
- Focus trap

**AdminForm.tsx** — Form wrapper:
- Обвива react-hook-form
- Auto error display
- Loading state на submit бутон
- Dirty state warning при навигация (prompt user)
...
- Customizable message и actions
- Danger variant за delete operations
...
Използвай custom hooks за data fetching и mutations:

```typescript
// hooks/usePosts.ts
function usePosts(params: PostListParams) {
  // useState за data, loading, error
  // useEffect за fetch
  // refetch function
  // optimistic updates
}
...
Или ако предпочиташ — `@tanstack/react-query` за по-мощен caching и mutation handling:
```bash
...
- `params` е `Promise` — винаги `await params`
- Client Components: маркирай с `'use client'` всеки component който използва hooks, event handlers или browser APIs
- Server Components за data fetching в page.tsx файлове
- `useRouter()` от `next/navigation` (НЕ от `next/router`)
- `useSearchParams()` за query string access в client components

...
Ти си backend developer. Проектът вече има структура, Docker setup, MySQL schema и Next.js scaffold, създадени от предишен агент. Твоята задача е да изградиш целия API layer.

## Стек

- **Next.js 16.1+** Route Handlers (App Router)
- **React 19.2.4+**
...
1. Прочети съществуващите файлове: `docker-compose.yml`, `mysql/init.sql`, `src/types/index.ts`, `src/lib/db.ts`
2. Инсталирай нужните зависимости:
   ```bash
   cd frontend
...
**Важно за Next.js 16:** `params` е async — винаги `await params` преди достъп до стойностите.
...
- Soft delete или hard delete (hard delete — CASCADE ще изтрие post_tags)
...
- `TIMESTAMP` колони имат range до 2038-01-19 — достатъчно за този проект

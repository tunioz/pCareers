exports.id=2676,exports.ids=[2676],exports.modules={35552:(a,b,c)=>{"use strict";let d;c.d(b,{Lf:()=>q,Rn:()=>p,YG:()=>m,Zy:()=>n,g7:()=>o});var e=c(87550),f=c.n(e),g=c(76760),h=c.n(g),i=c(73024),j=c.n(i);let k=process.env.DB_DIR||h().resolve(process.cwd(),"..","data"),l=process.env.DB_PATH||h().join(k,"pcloud.db");function m(a,b=[]){return d.prepare(a).all(...b)}function n(a,b=[]){return d.prepare(a).get(...b)}function o(a,b=[]){return d.prepare(a).run(...b)}function p(a){return d.transaction(a)()}function q(){return d}if("1"===process.env.NEXT_BUILD_MODE?d=new(f())(":memory:"):(j().existsSync(k)||j().mkdirSync(k,{recursive:!0}),d=new(f())(l)),d.pragma("journal_mode = WAL"),d.pragma("foreign_keys = ON"),d.pragma("busy_timeout = 5000"),d.exec(`
  ...
  `),"1"===process.env.NEXT_BUILD_MODE){let a=setInterval(()=>{try{let b=h().join(process.cwd(),".next","routes-manifest.json");if(j().existsSync(b)){clearInterval(a);let b=h().join(process.cwd(),".next","prerender-manifest.json");j().existsSync(b)||j().writeFileSync(b,JSON.stringify({version:4,routes:{},dynamicRoutes:{},staticRoutes:{},notFoundRoutes:[]})),setTimeout(()=>{try{d.close()}catch{}process.exit(0)},3e3)}}catch{}},5e3)}process.on("beforeExit",()=>{try{d.close()}catch{}})},78335:()=>{},96487:()=>{}};